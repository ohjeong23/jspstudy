...
<body>
  ...
  <script>
    ...
    document.getElementById('display').addEventListener('change',(evt)=>{
    	location.href = '${contextPath}/board/list.brd?page=1&sort=${sort}&display=' + evt.target.value; //20 50 100
    })
  </script>
        ...
        btnRemove.on('click', (evt)=>{
          ...
          $.each(chkEach, (i, elem)=>{
            if(elem.checked){          // if($(elem).is(':checked'))
              array.push(elem.value);//저장메소드
            }
          })//delete form board_t where board_no in (3,2,1);
          // array           === [3, 2, 1]
          // array.join(',') === '3,2,1'
