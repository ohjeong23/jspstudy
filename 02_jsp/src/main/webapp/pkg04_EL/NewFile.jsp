...
<%--
  ...
  2. Bind 영역에 저장된 값을 나타낼 수 있다.<div><%=a%></div> 바인드 영역에 저장되어 있지않아 나타나지 않음
 ...
 <%--
   Bind 영역 (데이터 저장 영역)
   1. pageContext : this(NewFile_jsp)  : 현재 JSP 인 NewFile.jsp에서만 접근 가능하다. 변형이 진짜되는가
 ...
 <%--
   JSP Bind 영역의 우선 순위
   1. 동일한 이름의 속성(Atrrtibute)이 여러 영역에 동시에 존재하는 경우 우선적으로 사용되는 경우가 있다.
   2. 높은 순위에서                 낮은 순위로
       pageContext > request > session > application  활용이 많을수록 낮은 순위
  --%>
<%--
